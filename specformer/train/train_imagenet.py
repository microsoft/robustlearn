...
from parser.parser_imagenet import get_args
...
print(args.model)
if args.model == "vit_small_patch16_224_in21k_sn":
    from model_for_imagenet_sn.vit import vit_small_patch16_224_in21k_sn
    model = vit_small_patch16_224_in21k_sn(img_size=crop_size,pretrained=(not args.scratch), num_classes=1000).cuda()
    ...
    model = vit_small_patch16_224_sn(img_size=crop_size,pretrained=(not args.scratch), num_classes=1000,pen_for_qkv=args.pen_for_qkv).cuda()
    model.init_sn(seed=args.seed);model = nn.DataParallel(model)
    logger.info('Model{}'.format(model))
elif args.model == "vit_base_patch16_224_in21k":
    from model_for_imagenet.vit import vit_base_patch16_224_in21k
    model = vit_base_patch16_224_in21k(img_size=crop_size,pretrained=(not args.scratch), num_classes=1000).cuda()
    model = nn.DataParallel(model)
    logger.info('Model{}'.format(model))
elif args.model == "vit_base_patch16_224_sn":
    from model_for_imagenet_sn.vit import vit_base_patch16_224_in21k_sn
    model = vit_base_patch16_224_in21k_sn(img_size=crop_size,pretrained=(not args.scratch), pen_for_qkv=args.pen_for_qkv, args=args,num_classes=1000).cuda()
    model.init_sn(seed=args.seed);model = nn.DataParallel(model)
    logger.info('Model{}'.format(model))
...
def train_adv(args, model, ds_train, ds_test, logger,epoch_s=0):
