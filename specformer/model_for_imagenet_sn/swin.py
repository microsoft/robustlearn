...
from timm.models.vision_transformer import checkpoint_filter_fn, get_init_weights_vit,init_weights_vit_timm
...
default_cfgs = {
    ...
    'swin_base_patch4_window7_224_sn': _cfg(
    ...
    'swin_small_patch4_window7_224_sn': _cfg(
        url='https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_small_patch4_window7_224.pth',
    ),

    'swin_tiny_patch4_window7_224_sn': _cfg(
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.,pen_for_qkv=None):
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,pen_for_qkv=None):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention(
            ...
            attn_drop=attn_drop, proj_drop=drop,pen_for_qkv=pen_for_qkv)
    ...
    def forward(self, x):
        ...
        # W-MSA/SW-MSA
        if self.training:
            attn_windows,spe_loss = self.attn(x_windows, mask=self.attn_mask)  
        else:
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0.,pen_for_qkv=None,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop,pen_for_qkv=pen_for_qkv,
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, weight_init='', pen_for_qkv=None,**kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            layers += [BasicLayer(
                ...
                use_checkpoint=use_checkpoint,pen_for_qkv=pen_for_qkv)
        ...
        if weight_init.startswith('jax'):
            for n, m in self.named_modules():
                get_init_weights_vit(m, n, head_bias=head_bias, jax_impl=True)
        else:
...
def swin_base_patch4_window7_224_sn(pretrained=False, **kwargs):
    """ Swin-B @ 224x224, pretrained ImageNet-22k, fine tune 1k
    """
    model_kwargs = dict(#patch_size=4,
         window_size=7, embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), **kwargs)
    return _create_swin_transformer('swin_base_patch4_window7_224_sn', pretrained=pretrained, **model_kwargs)
...
def swin_small_patch4_window7_224_sn(pretrained=False, **kwargs):
    """ Swin-S @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(#patch_size=4, 
        window_size=7, embed_dim=96, depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24), **kwargs)
    return _create_swin_transformer('swin_small_patch4_window7_224_sn', pretrained=pretrained, **model_kwargs)


def swin_tiny_patch4_window7_224_sn(pretrained=False, **kwargs):
    """ Swin-T @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(
         window_size=7, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), **kwargs)#patch_size=4,
    return _create_swin_transformer('swin_tiny_patch4_window7_224_sn', pretrained=pretrained, **model_kwargs)
