...
def checkpoint_filter_fn(state_dict, model,args):
    ...
    for k, v in state_dict.items():#他在reshape这个dict的权重
        # print(k)
        ...
        # elif 'attn.qkv.weight' in k: #随便给他一个初始化吧
        #     v = nn.init.xavier_normal_(v, gain=1.0)
        out_dict[k] = v
    for key in model.state_dict().keys():
        if not key in out_dict:
            _logger.warning('Initialized {}'.format(key))#但是其实他也没有initialize啊
    return out_dict #他就是过滤一下权重
